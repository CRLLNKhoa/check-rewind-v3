...
export default function Hlcalculate({extra=0,get,set}) {
  function tinhDauYeuCau(targetLv, currentLv) {
    ...
    // Biểu thức JavaScript tương đương với công thức Excel
    const result =
      extra !== 0
        ? extra * 22 +
          (2 *
            Math.log(
              Math.pow(
                2,
                Math.floor(Math.log10(targetLv)) - Math.floor(Math.log10(currentLv))
              ) *
                (targetLv / currentLv)
            )) /
            Math.log(1.065)
  ...
  }, [currentLv, currentLvDv, targetLv, targetLvDv, extra]);
  ...
  return (
    <div className="flex gap-4 items-center flex-wrap">
      ...
      <input
        ...
        type="number"
        className={cn(
          "outline-none cursor-not-allowed w-[102px] text-center text-yellow-500 bg-white/40 select-none font-bold"
        )}
