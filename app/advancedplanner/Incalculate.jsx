...
export default function Incalculate({ get, set, getMilestone, setMilestone, wt = false }) {
  ...
  useEffect(() => {
    if (currentLv !== 0 && currentLvDv !== 0 && targetLv !== 0 && targetLvDv) {
      ...
      set({...get, in: result.ketQuaElix})
    }
  }, [currentLv, currentLvDv, targetLv, targetLvDv,getMilestone.in]);
  ...
  return (
    <div className="flex gap-4 items-center flex-wrap">
      ...
      <input
         value={getMilestone.in}
         onChange={e => setMilestone({...getMilestone, in: e.target.value})}
         disabled={!wt}
        type="number"
        className={cn("outline-none cursor-not-allowed w-[102px] text-center text-green-500 bg-white/40 select-none font-bold", wt && "bg-transparent outline-none border-b w-[102px] text-center cursor-pointer")}
